...
| 6 | 探索结束后自动推进到**下个时间阶段** | 上午→下午→晚上→次日，时间资源化 |
...
- 每次探索**消耗一个时段**，无论探索是否完整走完 3 轮。
- 从 **Evening** 探索后进入 **Next Day Morning**，同时重置**每日探索次数**（回到 2 次）。
