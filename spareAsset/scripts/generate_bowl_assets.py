...
SRC = ROOT / "asset/mainScene/goods/bowl/bowls.png"
OUT_H = ROOT / "src/assets/BowlAssets.h"
OUT_CPP = ROOT / "src/assets/BowlAssets.cpp"
SPARE = ROOT / "spareAsset"
EXTRACTED = SPARE / "extracted/goods/bowl"
PREVIEWS = SPARE / "previews/goods"
GENERATED = SPARE / "generated/src_assets"

# Usage:
# 1. Put bowl source art in asset/mainScene/goods/bowl/bowls.png.
# 2. Keep each style as a separate connected object on transparent or green background.
# 3. Adjust BOWL_FRAME_W/BOWL_FRAME_H below when the in-game bowl size changes.
# 4. Run: python3 spareAsset/scripts/generate_bowl_assets.py
#
# The script writes src/assets/BowlAssets.*, extracted frames, a preview sheet,
...
BOWL_DESC_LINE2 = ["basic tray", "keeps food in,", "it blends well"]
BOWL_DESC_LINE3 = ["for daily feed.", "no spills.", "with nature."]
...
def make_frames():
    ...
    for i, (x1, y1, x2, y2) in enumerate(comps):
        ...
        scale = min(BOWL_FRAME_W / crop.width, BOWL_FRAME_H / crop.height)
        resized = crop.resize(
            (max(1, round(crop.width * scale)), max(1, round(crop.height * scale))),
            Image.Resampling.LANCZOS,
        )
        resized = trim_alpha(resized)
        canvas = Image.new("RGBA", (BOWL_FRAME_W, BOWL_FRAME_H), (0, 0, 0, 0))
        canvas.alpha_composite(resized, ((BOWL_FRAME_W - resized.width) // 2, BOWL_FRAME_H - resized.height))
        frames.append(canvas)
        canvas.save(EXTRACTED / f"bowl_{i}.png")
        print(f"bowl_{i}: {crop.width}x{crop.height} -> {resized.width}x{resized.height} in {BOWL_FRAME_W}x{BOWL_FRAME_H}")
    return frames
...
def write_preview(frames):
    sheet = Image.new("RGBA", (BOWL_FRAME_W * len(frames), BOWL_FRAME_H), (0, 0, 0, 0))
    for i, frame in enumerate(frames):
        sheet.alpha_composite(frame, (i * BOWL_FRAME_W, 0))
    sheet.save(PREVIEWS / "bowls_sheet_preview.png")
...
""",
    ...
    for frame in frames:
        ...
        data.extend(enc)
    cpp = ['#include "BowlAssets.h"', "", "namespace BowlAssets {", ""]
